$(document).ready(function() {
	...
	// 	$(this).attr('data-val', val);
	// })
	$('html, body').on('input', '.fr_youtube', function(){
		val = $(this).val().replace('https://', '').replace('www.', '').replace('youtube.com/watch?v=', '').replace('youtu.be/', '').replace('youtube.com/shorts/', '').replace(/\&.*/, '').replace(/\?.*/, '');
		$(this).attr('data-val', val);
	})
		...
		if (name.attr('data-pr') != 1 || phone.attr('data-pr') != 1) { mess('Введите свой данный') }
			...
			$.ajax({
				...
				success: function(data){
					if (data == 'yes') { 
						mess('Отправлено успешно')
					} else {
						mess('Пожалуйста, войдите снова!')
					}
				},
